...
namespace BoxIdDb
{
    public class ShSQL
    {
        ...
        static string conStringBoxidLPDDb = @"Server=192.168.145.4;Port=5432;User Id=pqm;Password=dbuser;Database=boxidlpddb; CommandTimeout=100; Timeout=100;";
        ...
        public string sqlExecuteScalarStringLPD(string sql)
        {
            string response;
            try
            {
                connection = new NpgsqlConnection(conStringBoxidLPDDb);
                connection.Open();
                NpgsqlCommand command = new NpgsqlCommand(sql, connection);
                response = Convert.ToString(command.ExecuteScalar());
                connection.Close();
        ...
        public bool sqlExecuteNonQueryLPD(string sql, bool result_message_show)
        {
            try
            {
                connection = new NpgsqlConnection(conStringBoxidLPDDb);
        ...
        public void sqlDataAdapterFillDatatableLPD(string sql, ref DataTable dt)
        {
            NpgsqlConnection connection = new NpgsqlConnection(conStringBoxidLPDDb);
            NpgsqlCommand command = new NpgsqlCommand();
        ...
        public bool sqlMultipleInsertOverallReturn(DataTable dt)
        {
            int res1;
            bool res2 = false;
            connection = new NpgsqlConnection(conStringBoxidDb);
        ...
        public bool sqlMultipleInsert(DataTable dt)
        {
            ...
            try
            {
                ...
                //connection.Open();
                string sql = "INSERT INTO product_serial(boxid, serialno, lot, line, process, linepass, testtime, model) " +
                    "VALUES (:boxid, :serialno, :lot, :line, :process, :linepass, :testtime, :model)";
                NpgsqlCommand command = new NpgsqlCommand(sql, connection);
